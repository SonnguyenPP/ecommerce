...
@section('sub-content')
<div>
    <h3 class="mb-4">{{ __("RESET YOUR PASSWORD") }}</h3>
    ...
    <form method="POST" action="{{ route('login.sendResetLink')}}" class="admin-reset-password">
        ...
        <div class="input-group mb-3">
            <input type="email" class="form-control py-2" value="{{ old('email') }}" name="email" placeholder="{{ __('Email') }}">
        </div>
        ...
        <p class="mb-2 text-muted">{{ __("Click here to") }} <a class="text-muted" href="{{ route('login') }}">{{ __("Log In") }}</a></p>
    </form>
 </div>
@endsection
