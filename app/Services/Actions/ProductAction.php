...
class ProductAction
{
    ...
    protected function saveProductVariation()
    {
        ...
        foreach ($arrayKeys as $i) {
            ...
            if (isActive('Inventory') && preference('manage_stock') == 1 && $variationData['variable_manage_stocks'][$i] == 1) {
                $locations['location'] = $variationData['location'][$i] ?? [];
                $adjust = $variationData['adjust'][$i] ?? [];
                \Modules\Inventory\Entities\StockManagement::productInitialStock($locations, $product);
                \Modules\Inventory\Entities\StockManagement::adjustVariation($adjust);
            }
            ...
            $product->manage_stocks = preference('manage_stock') == 1 && $variationData['variable_manage_stocks'][$i] != '' ? $variationData['variable_manage_stocks'][$i] : 0;
            $product->total_stocks = preference('manage_stock') == 1 && $variationData['variable_total_stocks'][$i] != '' ? $variationData['variable_total_stocks'][$i] : 0;
    ...
    protected function saveProductStock()
    {
        $stockData = [];
        if ($this->request->data) {
            parse_str($this->request->data, $stockData);
        }

        if (isActive('Inventory')) {
            $response = \Modules\Inventory\Entities\StockManagement::productInitialStock($stockData, $this->product);

            if ($response) {
                return $this->successResponse($response, 200, __('Stock updated successfully.'));
            }
        }
    }

    /**
     * inventory module stock update
     *
     * @return JsonResponse|void
     */
    protected function updateProductStock()
    {
        ...
        if (isActive('Inventory')) {
            $response = \Modules\Inventory\Entities\StockManagement::adjust($stockData, $this->product);
    ...
    protected function updateExtraProductData($request, $product)
    {
        ...
        if (isset($request['custom_fields'])) {
            CustomFieldService::storeFieldValue($request['custom_fields'], $product->id);
        }
