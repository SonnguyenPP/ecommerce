...
class MenuItemService
{
    ...
    private function getParentId(string|int $parent, $menuId = 1): int
    {
        ...
        if (! $menuItem) {
            $menuItem = MenuItems::where(['menu' => $menuId, 'depth' => 0, 'label->en' => $parent])->first();
        }
    ...
    public function addMenuItem(string|int $menuName, string $label, array $option): int|bool
    {
        try {
            ...
            $menuItem = MenuItems::where($condition)->first();
            
            if (! $menuItem) {
                $label = $condition['label'];
                unset($condition['label']);
                $menuItem = MenuItems::where($condition)->whereLike('label', '"en":"' . $label)->first();
            }
            ...
            return MenuItems::insertGetId($data);
        } catch (\Exception $e) {
            return false;
        }
    }
