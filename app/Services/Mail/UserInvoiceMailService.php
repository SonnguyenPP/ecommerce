...
class UserInvoiceMailService extends TechVillageMail
{
    ...
    {
        $this->sendEmailUser($request);

        if (! isset($request->only_user)) {
            $this->sendEmailUser($request, true);
        }
    }

    protected function sendEmailUser($request, $isSpecificMail = false)
    {
        if (! $isSpecificMail) {
            $email = $this->getTemplate(preference('dflt_lang'), 'order');
        } else {
            $email = $this->getTemplate(preference('dflt_lang'), 'order-specific-mail');
        }
        ...
        if (! $isSpecificMail) {
            ...
            $mailAddress = preference('order_notify_mail_address');
        ...
        if (! $isSpecificMail) {
            ...
            return $this->email->sendEmailWithAttachment($mailAddress, $subject, $message, $invoiceName, preference('company_name'));
        }
