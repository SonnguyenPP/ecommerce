        ...
        return datatables()
            ...
            ->addColumn('action', function ($reviews) {
                ...
                if ($this->hasPermission(['App\Http\Controllers\ReviewController@view'])) {
                    $str .= '<a data-bs-toggle="tooltip" title="' . __('Show') . '" href="' . route('review.view', ['id' => $reviews->id]) . '" class="action-icon me-1"><i class="feather icon-eye"></i></a>&nbsp;';
                }

                if ($this->hasPermission(['App\Http\Controllers\ReviewController@edit'])) {
                    $str .= '<a title="' . __('Edit') . '" href="' . route('review.edit', ['id' => $reviews->id]) . '" class="action-icon pr-5 edit_review" data-bs-toggle="modal" data-bs-target="#edit-review" data-id=' . $reviews->id . '><i class="feather icon-edit-1 neg-transition-scale-svg "></i></a>&nbsp;';
                }

                if ($this->hasPermission(['App\Http\Controllers\ReviewController@destroy'])) {
                    $str .= view('components.backend.datatable.delete-button', [
                        'route' => route('review.destroy', ['id' => $reviews->id]),
                        'id' => $reviews->id,
                    ])->render();
                }
