                 ...
                 <div class="md:w-{{ $totalCard == 1 ? 'full' : '1/' . $totalCard }} w-full">
                     <{{ $component->full_link == 1 ? 'a' : 'div' }} class="block relative"
                         ...
                         <div class="h-44" style="height: {{ $component->height . 'px' }}">
                             @isset($cta->image)
                                 <img class="h-full w-full object-fit neg-transition-scale {{ $component->rounded == 1 ? 'rounded-md' : '' }}"
                                     src="{{ Storage::disk()->url('public/uploads') . DIRECTORY_SEPARATOR . $cta['image'] }}">
                             @endisset
                         </div>
