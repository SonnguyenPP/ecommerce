...
@section('content')
    ...
    <div class="col-sm-12" id="purchase-add-container">
        ...
        <form action="{{ route('vendor.purchase.store') }}" method="post" id="purchase_form">
            @csrf
            <div class="col-sm-12">
                <div class="card">
...
@section('js')
    ...
    <script>
        var rowNo = 0;
        var totalAmount = 0;
        var addSupplierUrl = '{{ route('vendor.supplier.store') }}';
        var addLocation = '{{ route('vendor.location.store') }}'
    </script>
