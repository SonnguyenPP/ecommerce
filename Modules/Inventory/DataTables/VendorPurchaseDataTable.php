    ...
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->editColumn('reference', function ($purchases) {
                ...
                if ($this->hasPermission(['Modules\Inventory\Http\Controllers\Vendor\PurchaseController@receive'])) {
                    if ($purchases->status != 'Received') {
                        $str .= '<a title="' . __('Receive Inventory') . '" href="' . route('vendor.purchase.receive', ['id' => $purchases->id]) . '" class="action-icon"><i class="fas fa-cart-plus neg-transition-scale-svg "></i></a>&nbsp;';
                    }
                }

                if ($this->hasPermission(['Modules\Inventory\Http\Controllers\Vendor\PurchaseController@edit'])) {
                    $str .= '<a title="' . __('Edit') . '" href="' . route('vendor.purchase.edit', ['id' => $purchases->id]) . '" class="action-icon"><i class="feather icon-edit-1 neg-transition-scale-svg "></i></a>&nbsp;';
                }

                if ($this->hasPermission(['Modules\Inventory\Http\Controllers\Vendor\PurchaseController@destroy'])) {
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'action', 'name' => 'action', 'title' => '', 'width' => '12%',
                ...
                'visible' => $this->hasPermission(['Modules\Inventory\Http\Controllers\Vendor\PurchaseController@receive', 'Modules\Inventory\Http\Controllers\Vendor\PurchaseController@edit', 'Modules\Inventory\Http\Controllers\Vendor\PurchaseController@destroy']),
