    ...
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->editColumn('reference', function ($transfer) {
                ...
                if ($this->hasPermission(['Modules\Inventory\Http\Controllers\TransferController@receive'])) {
                    if ($transfer->status != 'Received') {
                        $str .= '<a title="' . __('Receive Inventory') . '" href="' . route('transfer.receive', ['id' => $transfer->id]) . '" class="action-icon"><i class="fas fa-cart-plus neg-transition-scale-svg "></i></a>&nbsp;';
                    }
                }

                if ($this->hasPermission(['Modules\Inventory\Http\Controllers\TransferController@edit'])) {
                    $str .= '<a title="' . __('Edit') . '" href="' . route('transfer.edit', ['id' => $transfer->id]) . '" class="action-icon"><i class="feather icon-edit-1 neg-transition-scale-svg "></i></a>&nbsp;';
                }

                if ($this->hasPermission(['Modules\Inventory\Http\Controllers\TransferController@destroy'])) {
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'action', 'name' => 'action', 'title' => '', 'width' => '12%',
                ...
                'visible' => $this->hasPermission(['Modules\Inventory\Http\Controllers\TransferController@receive', 'Modules\Inventory\Http\Controllers\TransferController@edit', 'Modules\Inventory\Http\Controllers\TransferController@destroy']),
