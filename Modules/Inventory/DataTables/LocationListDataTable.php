    ...
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->editColumn('name', function ($locations) {
                ...
                if ($this->hasPermission(['Modules\Inventory\Http\Controllers\LocationController@edit'])) {
                    $str = '<a title="' . __('Edit') . '" href="' . route('location.edit', ['id' => $locations->id]) . '" class="action-icon"><i class="feather icon-edit-1 neg-transition-scale-svg "></i></a>&nbsp;';
                }

                if ($this->hasPermission(['Modules\Inventory\Http\Controllers\LocationController@destroy'])) {
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'action', 'name' => 'action', 'title' => '', 'width' => '12%',
                ...
                'visible' => $this->hasPermission(['Modules\Inventory\Http\Controllers\LocationController@edit', 'Modules\Inventory\Http\Controllers\LocationController@destroy']),
