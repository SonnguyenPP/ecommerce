...
class PurchaseListDataTable extends DataTable
{
   ...
   */
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->editColumn('reference', function ($purchases) {
                return '<a href="' . route('purchase.edit', ['id' => $purchases->id]) . '">' . wrapIt($purchases->reference, 10, ['columns' => 4]) . '</a>';
            ...
            })->editColumn('vendor', function ($purchases) {
                return wrapIt(optional($purchases->vendor)->name, 20, ['columns' => 4]);
                ...
                if ($this->hasPermission(['Modules\Inventory\Http\Controllers\PurchaseController@receive'])) {
                    if ($purchases->status != 'Received') {
                        $str .= '<a title="' . __('Receive Inventory') . '" href="' . route('purchase.receive', ['id' => $purchases->id]) . '" class="action-icon"><i class="fas fa-cart-plus neg-transition-scale-svg "></i></a>&nbsp;';
                    }
                }

                if ($this->hasPermission(['Modules\Inventory\Http\Controllers\PurchaseController@edit'])) {
                    $str .= '<a title="' . __('Edit') . '" href="' . route('purchase.edit', ['id' => $purchases->id]) . '" class="action-icon"><i class="feather icon-edit-1 neg-transition-scale-svg "></i></a>&nbsp;';
                }

                if ($this->hasPermission(['Modules\Inventory\Http\Controllers\PurchaseController@destroy'])) {
                    $str .= '<form method="post" action="' . route('purchase.destroy', ['id' => $purchases->id]) . '" id="delete-purchase-' . $purchases->id . '" accept-charset="UTF-8" class="display_inline">
                        ' . csrf_field() . '
    ...
    */
    ...
    {
        $purchases = Purchase::select('id', 'reference', 'supplier_id', 'location_id', 'vendor_id', 'currency_id', 'arrival_date', 'total_amount', 'status')->with('location', 'supplier', 'currency', 'vendor')->filter();
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'action', 'name' => 'action', 'title' => '', 'width' => '12%',
                ...
                'visible' => $this->hasPermission(['Modules\Inventory\Http\Controllers\PurchaseController@receive', 'Modules\Inventory\Http\Controllers\PurchaseController@edit', 'Modules\Inventory\Http\Controllers\PurchaseController@destroy']),
