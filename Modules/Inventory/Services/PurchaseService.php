...
class PurchaseService
{
    ...
    protected function purchaseCalculations(): array
    {
        ...
        foreach ($request->product_id as $key => $productId) {
            ...
            $tax = (((float) $request->product_cost[$key] * (float) $request->product_qty[$key]) * (float) $request->product_tax[$key]) / 100;
            $totalTax += $tax;
            $totalQty += (float) $request->product_qty[$key];
            $totalAmount += ((float) $request->product_cost[$key] * (float) $request->product_qty[$key]) + $tax;
